...
module Actions =
    ...
    let execute (request: DriveBaseServiceRequest<'T>) =
        task { return! request.ExecuteAsync() }
        |> Async.AwaitTask
        |> Async.RunSynchronously

    let upload (request: FilesResource.CreateMediaUpload) =
        task { return! request.UploadAsync() }
        |> Async.AwaitTask
        |> Async.RunSynchronously
    ...
    let getFiles (driveService: DriveService) parentId mimeType =
        ...
        |> (fun r ->
            r.Q <- $"'{parentId}' in parents and mimeType='{mimeType}' and trashed=false"
            r)
        |> execute
    ...
    let createFolder (driveService: DriveService) parentId name =
        ...
        |> (fun f ->
            f.MimeType <- MimeType.Google(Folder)
            driveService.Files.Create(f))
        |> execute
